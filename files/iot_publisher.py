...
Implementa throttling configurable para no superar el límite de mensajes
y evitar costos imprevistos.

Payload de cada evento:
    {
        "timestamp": "2026-06-08T12:00:00Z",  # ISO 8601 UTC
        "session_id": "abc123",
        "clase_detectada": 1,
...
from config.settings import (
    ...
    AWS_REGION, MQTT_THROTTLE_SECONDS,
)
...
class IoTPublisher:
    ...
    En entornos sin credenciales AWS, los eventos se loguean localmente.
    """
    ...
    def _init_mqtt_client(self):
        """Inicializa el cliente MQTT de awsiotsdk o retorna None."""
    ...
    def publish_event(
        ...
        """
        Publica un evento de clasificación en IoT Core.
        Respeta el throttling: no publica si no pasaron MQTT_THROTTLE_SECONDS.

        Returns:
            True si se publicó, False si se omitió por throttle o error.
        """
        now = time.monotonic()
        ...
        try:
            ...
            logger.debug("Evento publicado en IoT Core: clase=%d", class_id)
            return True
        except Exception as exc:
