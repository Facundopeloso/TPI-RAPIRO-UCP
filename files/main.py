...
def main() -> None:
    ...
    with CameraCapture() as camera:
        ...
        for frame in camera.frames():
            ...
            if result is None:
                # Confianza insuficiente — no actuar
                continue

            # Actuación física
            rapiro.react(result.class_id)

            # Seguimiento de ausencia prolongada
            if result.class_id == CLASS_ABSENT:
                if absence_start is None:
                    ...
                    absence_start = None  # Reset para no re-alertar inmediatamente
            else:
                absence_start = None
            ...
            logger.debug(
                "Frame procesado: clase=%d ('%s') conf=%.2f lat=%.1fms",
                result.class_id, result.label, result.confidence, result.latency_ms,
            )
